...
def generate_transactions_table(customer_profile, start_date="2021-05-01", nb_days=10):
...
def generate_dataset(n_customers=10000, n_terminals=1000000, nb_days=90, start_date="2021-05-01", r=5):
...
def save_dataset(transactions_df, DIR_OUTPUT="simulated-data-raw/data/"):
    if not os.path.exists(DIR_OUTPUT):
        os.makedirs(DIR_OUTPUT)

    start_date = datetime.datetime.strptime("2021-05-01", "%Y-%m-%d")

    for day in range(transactions_df.TX_TIME_DAYS.max() + 1):
        transactions_day = transactions_df[transactions_df.TX_TIME_DAYS == day].sort_values('TX_TIME_SECONDS')
        transactions_day['TX_DATETIME'] = transactions_day['TX_DATETIME'].apply(lambda x: x.strftime('%Y-%m-%d %H:%M:%S'))
